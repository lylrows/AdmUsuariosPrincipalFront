...
export class JwtAuthService {
  ...
  public signin(username, password) {
    ...
    this.signingIn = true;
    return this.http.post(`${environment.apiURL}/Login/authenticate`, { username, password })
      .pipe(
        map((res: any) => {
          
          if (res.stateCode== 200 && res.data.isLoginOk){
              this.setUserAndToken(res.data.token, res.data.user, !!res,res.data.menu.items);
          }
  ...
  */
  ...
  setUserAndToken(token: String, user: User, isAuthenticated: Boolean, menu: any) {
    ...
    this.ls.setItem(this.APP_MENU, menu);
  }
}

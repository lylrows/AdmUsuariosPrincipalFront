...
import { DialogPostulantInternalComponent } from '../recruitment-internal/dialog-postulant-internal/dialog-postulant-internal.component';
import { MastertableService } from '@app/data/service/mastertable.service';
import { filterPostulant } from '@app/data/schema/Postulant/filterPostulant';
import { DialogPostulantFileComponent } from '../recruitment-external/recruitment-evaluation/dialog-postulant-file/dialog-postulant-file.component';
import { PostulantFileService } from '@app/data/service/PostulantFileService';


@Component({
  selector: 'app-recruitment-person',
  templateUrl: './recruitment-person.component.html',
  styleUrls: ['./recruitment-person.component.scss']
})
export class RecruitmentPersonComponent implements OnInit {
    ...
    filter:filterPostulant = new filterPostulant();
    typeFilter= [
      {id:1,sname:'EXTERNA'},
      {id:2,sname:'INTERNA'}
    ]
    ...
    ngOnInit() {
      ...
      this.filter.ntypefilter = 1;
      this.filter.typedocument = null;
      this.getEvaluation();
      this.screenWidth = window.innerWidth;
    }
    
    getEvaluation() {
      this.evaluationService.GetEvaluationPostulantsAll(this.filter).subscribe((res) => {
        setTimeout(() => {
          this.flowDT.data = res.data;
    ...
    getInfo() {
      ...
      if(this.filter.typedocument > 0 && (this.filter.numberdocumen == "" || this.filter.numberdocumen.length < 8)){
        this.snack.open('Debe ingresar numero de documento', "Info", { duration: 4000 });
        return false;
      }
      this.filter.ntypefilter = parseInt(this.filter.ntypefilter.toString());
      this.evaluationService.GetEvaluationPostulantsAll(this.filter).subscribe((res) => {
        setTimeout(() => {
          this.flowDT.data = res.data;
          //this.showSelectPosition = res.dataDetail;
    ...
    openFilePostulant(data: any = {}) {    
      ...
      if (data.stype == 'EXTERNA') {
        this.postulantFileService.getinfopersonexactus(data.nid_postulant).subscribe((res) => {
          let dataToSend = res.data;
          dataToSend.isAdministration = true;
          dataToSend.permitConfirmar = true;
          dataToSend.permitSendExactus = environment.perfilAsistenteRRHH.indexOf(storage.nid_profile) == -1;
          let dialogRef: MatDialogRef<any> = this.dialog.open(DialogPostulantFileComponent, {
            ...
            maxWidth : "100vw",
            disableClose: true,
            data: { title: title, payload: {idEvaluation: data.nid_evaluation, data: dataToSend} },
          });
    ...
    this.filterPostulantRequest.idPostulant = this.data.payload.data.idPostulant;*/
        let dataToSend = {
          idEvaluation: data.nid_evaluation,
          idPostulant: data.nid_postulant,
          isAdministration: true,
          permitConfirmar: true,
          permitSendExactus: environment.perfilAsistenteRRHH.indexOf(storage.nid_profile) == -1
        }; 
    ...
    changeTipoPostulante() {
      this.disabledStatus = this.filter.ntypefilter == 2;
      if (this.disabledStatus) this.filter.nstate = '0';
    }
  }
